...
/// Creates a validated FDT view from the firmware-provided physical address.
///
/// # Safety
///
/// The Raspberry Pi firmware must guarantee that:
///
/// - `dtb_address` points to a readable FDT header.
/// - The DTB remains readable and immutable during boot.
/// - The advertised `totalsize` describes readable contiguous memory.
...
pub(crate) unsafe fn from_address(dtb_address: usize) -> Result<Fdt<'static>, Error> {
    ...
    // SAFETY: The caller guarantees that this physical range is readable,
    // contiguous, immutable, and valid throughout boot.
